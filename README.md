...
**Минимальный сценарий — просто скинь ссылку:**

1. Отправь боту ссылку на любой подарок:
   - с маркета — `https://tonnel.network/gift/12345`
     (работает для Tonnel, MRKT, Portals, Fragment)
   - ссылку из Telegram — `https://t.me/nft/WinterWreath-2868`
     (коллекция определяется по слагу ссылки)
2. Бот сам определит коллекцию и сразу начнёт отслеживать похожие подарки
3. Кнопка **⚙️ Настроить алерт** — подкрутить: мин. % прибыли,
   рынки продажи, пауза
4. Кнопки меню — в строке ввода под полем сообщения (Reply Keyboard):
   **🔍 Настроить поиск · 📊 Мои алерты · ⏸️ Пауза/▶️ Продолжить · ℹ️ Помощь**
...
- [ ] Уточнение модели/фона/узора по одной ссылке (сейчас — только коллекция)
